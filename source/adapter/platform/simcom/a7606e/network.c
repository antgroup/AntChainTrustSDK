...
/**
 * @file network.c
 * @brief SIMCom A7606E-H platform network adapter implementation
 *
 * Implements the AntChainTrustSDK network interface using POSIX sockets.  The
 * A7606E-H runs OpenWrt Linux on an ASR1806 SoC with musl libc, providing
 * standard BSD socket APIs.  Timeout management is based on poll() with
 * monotonic-clock EINTR recovery.  TCP connect uses non-blocking connect + poll
 * + SO_ERROR.
 *
