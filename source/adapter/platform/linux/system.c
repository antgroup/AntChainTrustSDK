...
/**
 * @file system.c
 * @brief Linux platform system adapter implementation
 *
 * Provides cross-platform primitives for time, mutexes, semaphores, threads,
 * and log output. Adapted for POSIX/Linux environment with comprehensive error
...
actrust_err_t actrust_log_out(const char *buf, size_t len)
{
    ...
    while (total < len) {
        ...
        written = write(STDOUT_FILENO, buf + total, chunk);
        if (written < 0) {
            if (errno == EINTR) {
                continue;
            }
