...
{
    char ip[16] = { 0 };
    TEST_ASSERT_EQUAL(ACTRUST_OK, actrust_net_dns_resolve("example.com", ip,
                                                          sizeof(ip), 5000));
...
void test_tcp_echo(void)
{
    ...
    actrust_err_t err = actrust_net_connect(sock, ip, 80, 5000);
    if (!ACTRUST_IS_OK(err)) {
        actrust_net_close(sock);
        TEST_IGNORE_MESSAGE("network unavailable, skipping tcp echo");
    }

    const char *req     = "HEAD / HTTP/1.0\r\nHost: example.com\r\n\r\n";
    size_t      req_len = strlen(req);
    size_t      sent    = 0;
    TEST_ASSERT_EQUAL(ACTRUST_OK, actrust_net_send(sock, (const uint8_t *) req,
                                                   req_len, 5000, &sent));
    ...
    TEST_ASSERT_EQUAL(ACTRUST_OK,
                      actrust_net_recv(sock, rx, sizeof(rx) - 1, 5000, &got));
...
{
    char ip[16] = { 0 };
    TEST_ASSERT_EQUAL(ACTRUST_OK, actrust_net_dns_resolve("pool.ntp.org", ip,
                                                          sizeof(ip), 5000));
    ...
    size_t        sent = 0;
    actrust_err_t err  = actrust_net_sendto(sock, ip, 123, ntp_req,
                                            sizeof(ntp_req), 5000, &sent);
    if (!ACTRUST_IS_OK(err)) {
        actrust_net_close(sock);
        TEST_IGNORE_MESSAGE("network unavailable, skipping udp sendrecv");
    }
